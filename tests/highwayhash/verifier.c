...
static const uint64_t kTestKey1[4] = {
    0x0706050403020100ull, 0x0F0E0D0C0B0A0908ull, 0x1716151413121110ull,
    0x1F1E1D1C1B1A1918ull};

static const uint64_t kTestKey2[4] = {1ull, 2ull, 3ull, 4ull};

static const uint64_t kExpected64[kMaxSize + 1] = {
...
static bool TestHash64(HighwayHash64_t fn, uint64_t expected,
                       const uint8_t *data, size_t size, const uint64_t *key) {
  uint64_t hash = fn(key, data, size);
  if (expected != hash) {
    if (!is_option_set(test_quiet))
      printf("Failed! : expected %016" PRIx64 ", got %016" PRIx64
             ", size: %d\n",
