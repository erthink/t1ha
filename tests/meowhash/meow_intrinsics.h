...
#define Meow128_AreEqual(A, B)                                                 \
  (_mm_movemask_epi8(_mm_cmpeq_epi8((A), (B))) == 0xFFFF)
#define Meow128_AESDEC(Prior, Xor) _mm_aesdec_si128((Prior), (Xor))
#define Meow128_AESDEC_Mem(Prior, Xor)                                         \
  _mm_aesdec_si128((Prior), _mm_loadu_si128((meow_u128 *)(Xor)))
...
// TODO(casey): Not sure if this should actually be Meow128_Zero(A) ((A) =
// _mm_setzero_si128()), maybe
...
#define Meow256_AESDEC_Mem(Prior, XOr)                                         \
  _mm256_aesdec_epi128((Prior), *(meow_u256 *)(XOr))
#define Meow256_Store(Value, Ptr)                                              \
  _mm256_store_si256((meow_u256 *)(Ptr), (Value));
#define Meow256_Zero() _mm256_setzero_si256()

#define Meow512_AESDEC(Prior, XOr) _mm512_aesdec_epi128((Prior), (XOr))
#define Meow512_AESDEC_Mem(Prior, XOr)                                         \
  _mm512_aesdec_epi128((Prior), *(meow_u256 *)(XOr))
#define Meow512_Store(Value, Ptr)                                              \
  _mm256_store_si256((meow_u512 *)(Ptr), (Value));
...
// Thus on ARMv8 it requires extra InvMixColumns call and delay on Xor
// operation. On iteration N it needs to use m[N-1] as input, and remeber m[N]
// for next iteration. This structure will store memory operand in member B
// which will be used in next AESDEC opcode. Remember to do one more XOR(A,B)
// when finishing AES operations in a loop.
typedef struct {
  meow_u128 A;
  meow_u128 B;
} meow_aes_128;

static int Meow128_AreEqual(meow_u128 A, meow_u128 B) {
  uint8x16_t Powers = {
...
static meow_aes_128 Meow128_AESDEC(meow_aes_128 Prior, meow_u128 Xor) {
  meow_aes_128 R;
...
static meow_aes_128 Meow128_AESDEC_Mem(meow_aes_128 Prior, void *Xor) {
  meow_aes_128 R;
  R.A = vaesimcq_u8(vaesdq_u8(Prior.A, Prior.B));
...
static meow_u128 Meow128_AESDEC_Finalize(meow_aes_128 Value) {
  meow_u128 R = veorq_u8(Value.A, Value.B);
  return (R);
}

static meow_u128 Meow128_Zero() {
  meow_u128 R = vdupq_n_u8(0);
  return (R);
}

static meow_aes_128 Meow128_ZeroState() {
  meow_aes_128 R;
  R.A = R.B = vdupq_n_u8(0);
  return (R);
}

static meow_u128 Meow128_Set64x2(meow_u64 Low64, meow_u64 High64) {
  meow_u128 R = vreinterpretq_u8_u64(
      vcombine_u64(vcreate_u64(Low64), vcreate_u64(High64)));
  return (R);
}

static meow_aes_128 Meow128_Set64x2_State(meow_u64 Low64, meow_u64 High64) {
  meow_aes_128 R;
...
typedef meow_u128 meow_hash_implementation(meow_u64 Seed, meow_u64 Len,
                                           void *Source);
typedef void meow_absorb_implementation(struct meow_hash_state *State,
                                        meow_u64 Len, void *Source);
